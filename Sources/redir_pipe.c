...
static int	ft_count_words(char *str, char *delim, int nwords)
{
	int quotes[2];
	int i;
	...
	while (str && str[i] != '\0')
	{
		...
		{
			while ((!ft_strchr(delim, str[i]) || quotes[0] || quotes[1]) && str[i] != '\0')
			{
...
static char	**ft_fill_matrix(char **tmp_matrix, char *str, char *delim, int i[3])
{
	int quotes[2];
...
char	**handle_pipe_redir(char const *str, char *delim)
{
	...
	if (!str)
		return (NULL);
	nwords = ft_count_words((char *)str, delim, 0);
	if (nwords == -1)
		return (NULL);
	tmp_matrix = malloc((nwords + 1) * sizeof(char *));
	if (tmp_matrix == NULL)
		return (NULL);
	tmp_matrix = ft_fill_matrix(tmp_matrix, (char *)str, delim, i);
